  ...
  return (
    <main className="bg-white text-black dark:bg-black dark:text-white min-h-screen font-sans">
      ...
      <section className="py-20 px-6 sm:px-12 lg:px-20 bg-gray-100 dark:bg-gradient-to-br dark:from-gray-950 dark:to-black">
        <div className="max-w-3xl mx-auto mb-10 relative">
          <Search className="absolute left-4 top-3 text-gray-500 dark:text-gray-400" />
          <input
            type="text"
            placeholder="Search featured listings..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full py-3 pl-12 pr-4 rounded-lg bg-white dark:bg-white/5 border border-gray-300 dark:border-gray-700 text-black dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-600"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredCars.map((car) => (
            <div
              key={car.id}
              className="bg-white dark:bg-white/5 backdrop-blur-lg rounded-2xl border border-gray-200 dark:border-white/10 p-4 shadow-md hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02]"
            >
              <div className="rounded-xl overflow-hidden mb-4">
                <img
                  src={car.image}
        ...
        {filteredCars.length === 0 && (
          <div className="text-center text-gray-500 dark:text-gray-400 mt-16">No matching cars found.</div>
        )}
      </section>
